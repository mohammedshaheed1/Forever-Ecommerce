...
//              try {
//                 const {data}=await api.post('/api/order/verifyRazorpay',response,{headers:{token}})
//                 if(data.success){
//                    navigate('/order')
//                    setCartItems({})
//                 }
...
const PlaceOrder = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      for (const items in cartItems) {
        for (const item in cartItems[items]) {
          if (cartItems[items][item] > 0) {
            ...
            if (product) {
              let itemData = structuredClone(product);
              itemData.size = item;
      ...
      switch (method) {
        ...
        case "stripe": {
          const res = await api.post("/api/order/stripe", orderData, {
            headers: { token },
          });
          if (res.data.success) {
        ...
        case "razorpay": {
          const res = await api.post("/api/order/razorpay", orderData, {
            headers: { token },
          });
          if (res.data.success) {
  ...
  return (
    <form
      ...
      className="flex flex-col lg:flex-row justify-between gap-10 pt-6 min-h-[80vh] border-t max-w-6xl mx-auto px-4"
    >
      {/* --- Left Section (Delivery Info) --- */}
      <div className="flex flex-col gap-5 w-full lg:max-w-[480px] bg-white p-6 rounded-xl shadow-sm border">
        <Title text1="DELIVERY" text2="INFORMATION" />
        ...
        <div className="flex gap-4">
          <input
            ...
            className="border border-gray-300 rounded-lg px-4 py-2 w-full focus:ring focus:ring-gray-200"
          />
          <input
            required
            onChange={onChangeHandler}
      ...
      <div className="flex-1">
        ...
        <div className="mt-10 bg-white p-6 rounded-xl shadow-sm border">
          ...
          <button
            type="submit"
            className="mt-6 w-full bg-black text-white py-3 rounded-lg text-sm font-semibold hover:bg-gray-900 transition"
          >
            PLACE ORDER
          </button>
        </div>
